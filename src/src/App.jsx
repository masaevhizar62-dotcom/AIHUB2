...
const target = prev.find(a => a.id === id);
  ...
  const toggleVoiceInput = useCallback(() => {
    triggerHaptic();
    if (isVoiceListening) { setIsVoiceListening(false); return; }
    const SpeechRecognition = window.SpeechRecognition || window.webkitSpeechRecognition;
    if (!SpeechRecognition) {
      setIsVoiceListening(true);
      setTimeout(() => {
        setInputText(prev => prev + (prev ? ' ' : '') + 'Напиши сценарий для короткого видеоролика о новинках ИИ');
  ...
  }, [isVoiceListening, triggerHaptic]);

  const handleSendPrompt = useCallback((targetAppOverride = null) => {
    const targetApp = targetAppOverride || currentApp;
    if (!inputText.trim() && attachments.length === 0) return;
    triggerHaptic();
    if (!targetApp.isInstalled) { setNotInstalledModal(targetApp); return; }

    const newHistoryItem = { id: `h_${Date.now()}`, appId: targetApp.id, text: inputText, timestamp: Date.now(), attachments: [...attachments] };
    ...
    setShowAttachMenu(false);
    setShowSlashMenu(false);
  }, [currentApp, inputText, attachments, triggerHaptic]);

  const copyToClipboard = useCallback((text, id = null) => {
    triggerHaptic();
    navigator.clipboard?.writeText(text);
    if (id) { setCopiedId(id); setTimeout(() => setCopiedId(null), 1800); }
  }, [triggerHaptic]);

  const resendFromHistory = useCallback((item, newAppId) => {
  ...
  const handleAddNewTemplate = useCallback((e) => {
    e.preventDefault();
    if (!newTemplateTitle.trim() || !newTemplateText.trim()) return;
    setTemplates(prev => [{ id: `t_${Date.now()}`, title: newTemplateTitle, text: newTemplateText, tag: 'Пользовательский' }, ...prev]);
    setNewTemplateTitle('');
    setNewTemplateText('');
    setShowAddTemplateModal(false);
    triggerHaptic();
  }, [newTemplateTitle, newTemplateText, triggerHaptic]);
  ...
  const accent = ACCENT_COLORS[accentKey] || ACCENT_COLORS.monochrome;

  const filteredHistory = useMemo(() => {
    const q = historyQuery.toLowerCase();
    if (!q) return history;
    return history.filter(h => h.text.toLowerCase().includes(q) || apps.find(a => a.id === h.appId)?.name.toLowerCase().includes(q));
  }, [history, historyQuery, apps]);

  const recentBubbles = useMemo(() => history.slice(0, 12), [history]);
  ...
  return (
    <div className="flex flex-col h-screen max-w-md mx-auto bg-zinc-50 dark:bg-zinc-950 text-zinc-900 dark:text-zinc-100 font-sans antialiased overflow-hidden border-x border-zinc-200 dark:border-zinc-800 shadow-2xl relative select-none [-webkit-tap-highlight-color:transparent]">
      <style>{`
        ...
        .cv-auto { content-visibility: auto; contain-intrinsic-size: 1px 90px; }
      ...
      {/* TOP BAR — Claude-style: plain title on chat, back chevron + section title everywhere else */}
      <header className="relative flex items-center justify-between px-2 py-2.5 bg-zinc-50/90 dark:bg-zinc-950/90 backdrop-blur-md border-b border-zinc-200/70 dark:border-zinc-800/70 z-20 shrink-0 h-[52px]">
        {activeTab === 'chat' ? (
          <>
            <div className="flex items-center gap-1 pl-1.5">
              <span className="text-[15px] font-semibold tracking-tight">AI Hub</span>
              <ChevronDown className="w-3.5 h-3.5 text-zinc-400 mt-0.5" />
            </div>
            <div className="flex items-center">
              {[
                ...
                <button
                  ...
                  className={`p-2.5 rounded-full text-zinc-500 dark:text-zinc-400 ${TAP}`}
                >
          ...
          <>
            <button
              onClick={() => { setActiveTab('chat'); triggerHaptic(); }}
              className={`flex items-center gap-0.5 pl-1 pr-3 py-2 rounded-full text-zinc-700 dark:text-zinc-200 ${TAP}`}
              aria-label="Назад"
            >
              <ChevronLeft className="w-5 h-5" />
              <span className="text-[15px]">Назад</span>
            </button>
            <span className="absolute left-1/2 -translate-x-1/2 text-[15px] font-semibold tracking-tight">
              {activeTab === 'history' ? 'История' : activeTab === 'templates' ? 'Шаблоны' : 'Настройки'}
            </span>
            <div className="w-[74px] flex justify-end">
              {activeTab === 'history' && history.length > 0 && (
                <button onClick={clearHistory} className={`p-2.5 rounded-full text-zinc-400 hover:text-red-500 ${TAP}`}>
                  <Trash2 className="w-[18px] h-[18px]" />
                </button>
              )}
              {activeTab === 'templates' && (
                <button onClick={() => setShowAddTemplateModal(true)} className={`p-2.5 rounded-full text-zinc-500 dark:text-zinc-400 ${TAP}`}>
      ...
      {/* MAIN CONTENT */}
      <main className="flex-1 overflow-y-auto relative flex flex-col overscroll-contain">
        {activeTab === 'chat' && (
          <div className="flex-1 p-4 flex flex-col justify-end min-h-full">
            {recentBubbles.length === 0 ? (
              <div className="flex-1 flex flex-col items-center justify-center text-center gap-2 opacity-70">
                ...
                <p className="text-[13px] text-zinc-400 max-w-[220px]">Выберите приложение внизу, напишите запрос и отправьте — он откроется прямо в выбранном ИИ</p>
              </div>
            ) : (
              <div className="space-y-4">
                {[...recentBubbles].reverse().map(item => (
                  <PromptBubble key={item.id} item={item} app={appsById[item.appId]} onCopy={copyToClipboard} copiedId={copiedId} />
                ))}
              </div>
            )}
          </div>
        ...
        {activeTab === 'history' && (
          <div className="flex-1 p-4 flex flex-col gap-3">
            <div className="flex items-center gap-2">
              <div className="flex-1 flex items-center gap-2 bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 rounded-xl px-3 py-2">
              ...
              {history.length > 0 && (
                <button onClick={clearHistory} className={`p-2 rounded-xl text-zinc-400 hover:text-red-500 border border-zinc-200 dark:border-zinc-800 ${TAP}`}>
                  <Trash2 className="w-4 h-4" />
                </button>
              )}
            </div>
            {filteredHistory.length === 0 ? (
              <p className="text-center text-[13px] text-zinc-400 mt-10">Ничего не найдено</p>
            ) : (
              <div className="space-y-2">
                {filteredHistory.map(item => (
                  <div key={item.id} className="cv-auto">
                    <HistoryRow item={item} app={appsById[item.appId]} apps={apps} onResend={resendFromHistory} onCopy={copyToClipboard} copiedId={copiedId} />
                  </div>
                ))}
              </div>
            )}
        ...
        {activeTab === 'templates' && (
          <div className="flex-1 p-4 flex flex-col gap-3">
            <div className="space-y-2">
              {templates.map(t => (
                <TemplateCard key={t.id} template={t} onUse={useTemplate} onDelete={deleteTemplate} />
              ))}
            </div>
          </div>
        )}
        ...
        {activeTab === 'settings' && (
          <div className="flex-1 p-4 flex flex-col gap-6">
            <section>
              <h2 className="text-[12px] font-semibold uppercase tracking-wide text-zinc-400 mb-2">Оформление</h2>
              <div className="flex gap-2 mb-3">
                <button onClick={() => setTheme('light')} className={`flex-1 flex items-center justify-center gap-1.5 py-2.5 rounded-xl border text-[13px] font-medium ${TAP} ${theme === 'light' ? 'border-zinc-900 dark:border-zinc-100 bg-zinc-100 dark:bg-zinc-800' : 'border-zinc-200 dark:border-zinc-800 text-zinc-500'}`}>
                  <Sun className="w-4 h-4" /> Светлая
                </button>
                <button onClick={() => setTheme('dark')} className={`flex-1 flex items-center justify-center gap-1.5 py-2.5 rounded-xl border text-[13px] font-medium ${TAP} ${theme === 'dark' ? 'border-zinc-900 dark:border-zinc-100 bg-zinc-100 dark:bg-zinc-800' : 'border-zinc-200 dark:border-zinc-800 text-zinc-500'}`}>
              ...
              <div className="flex gap-2">
                {Object.entries(ACCENT_COLORS).map(([key, val]) => (
                  <button
                    ...
                    className={`w-8 h-8 rounded-full border-2 ${TAP} ${accentKey === key ? 'border-zinc-900 dark:border-zinc-100' : 'border-transparent'}`}
            ...
            <section>
              <h2 className="text-[12px] font-semibold uppercase tracking-wide text-zinc-400 mb-2">Приложения ИИ</h2>
              <div className="space-y-1.5">
                {orderedApps.map(a => (
                  <div key={a.id} className="flex items-center justify-between bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 rounded-xl px-3 py-2.5">
                    <div className="flex items-center gap-2.5 min-w-0">
                      <AppLogo appId={a.id} className="w-5 h-5 shrink-0" />
                    ...
                    <div className="flex items-center gap-1 shrink-0">
                      <button onClick={() => toggleInstalled(a.id)} className={`text-[11px] px-2 py-1 rounded-lg ${TAP} ${a.isInstalled ? 'bg-emerald-500/10 text-emerald-600 dark:text-emerald-400' : 'bg-zinc-100 dark:bg-zinc-800 text-zinc-400'}`}>
                        {a.isInstalled ? 'Установлено' : 'Нет'}
                      </button>
                      <button onClick={() => togglePin(a.id)} className={`p-1.5 rounded-lg ${TAP} ${a.isPinned ? 'text-amber-500' : 'text-zinc-300'}`}>
                        {a.isPinned ? <Pin className="w-4 h-4" /> : <PinOff className="w-4 h-4" />}
                      </button>
                    </div>
                  </div>
                ))}
            ...
            <section>
              <h2 className="text-[12px] font-semibold uppercase tracking-wide text-zinc-400 mb-2">Приложение по умолчанию</h2>
              <div className="flex flex-wrap gap-2">
                {apps.map(a => (
                  <AppChip key={a.id} app={a} isSelected={selectedAppId === a.id} onSelect={setSelectedAppId} />
                ))}
              </div>
            </section>
          </div>
      ...
      {/* APP PICKER + COMPOSER (chat tab only) */}
      {activeTab === 'chat' && (
        <div className="shrink-0 border-t border-zinc-200 dark:border-zinc-800 bg-zinc-50/95 dark:bg-zinc-950/95 backdrop-blur-md">
          {/* attachments preview */}
          {attachments.length > 0 && (
            <div className="flex gap-2 px-4 pt-3 overflow-x-auto">
              {attachments.map(a => (
                <div key={a.id} className="relative shrink-0 w-16 h-16 rounded-xl overflow-hidden bg-zinc-200 dark:bg-zinc-800 flex items-center justify-center">
                  {a.previewUrl ? (
                  ...
                  <button onClick={() => removeAttachment(a.id)} className={`absolute top-0.5 right-0.5 bg-black/60 rounded-full p-0.5 ${TAP}`}>
          ...
          {/* slash command menu */}
          {showSlashMenu && availableSlashCommands.length > 0 && (
            <div className="mx-4 mt-2 bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 rounded-xl overflow-hidden animate-[slideUp_150ms_ease-out]">
              {availableSlashCommands.map(cmd => (
                <button key={cmd.command} onClick={() => applySlashCommand(cmd)} className={`w-full flex items-center justify-between px-3 py-2.5 text-left hover:bg-zinc-50 dark:hover:bg-zinc-800 border-b last:border-b-0 border-zinc-100 dark:border-zinc-800 ${TAP}`}>
                  <div>
                    <p className="text-[13px] font-medium text-zinc-800 dark:text-zinc-100">{cmd.command}</p>
                    <p className="text-[11px] text-zinc-400">{cmd.label}</p>
                  </div>
                </button>
              ))}
            </div>
          ...
          {/* app picker — mirrors Claude's model-selector pill */}
          <div className="px-4 pt-3">
            <button
              onClick={() => { setShowAppSheet(true); triggerHaptic(); }}
              className={`flex items-center gap-1.5 pl-2 pr-2.5 py-1.5 rounded-full border border-zinc-200 dark:border-zinc-800 bg-white dark:bg-zinc-900 ${TAP}`}
            >
              <AppLogo appId={currentApp?.id} className="w-4 h-4" />
              <span className="text-[13px] font-medium text-zinc-700 dark:text-zinc-200">{currentApp?.name}</span>
          ...
          {/* attach menu */}
          {showAttachMenu && (
            <div className="flex gap-2 px-4 pt-3 animate-[slideUp_150ms_ease-out]">
              {[
                { icon: ImageIcon, label: 'Фото', accept: 'image/*' },
                { icon: VideoIcon, label: 'Видео', accept: 'video/*' },
                { icon: FileText, label: 'Файл', accept: '*' },
              ].map(({ icon: Icon, label, accept }) => (
                <button
                  key={label}
                  onClick={() => { fileInputRef.current.accept = accept; fileInputRef.current.click(); }}
                  className={`flex-1 flex flex-col items-center gap-1 py-2.5 rounded-xl border border-zinc-200 dark:border-zinc-800 bg-white dark:bg-zinc-900 ${TAP}`}
                >
          ...
          <div className="flex items-end gap-2 px-4 py-3">
            <button onClick={() => { setShowAttachMenu(s => !s); triggerHaptic(); }} className={`p-2.5 rounded-full border border-zinc-200 dark:border-zinc-800 text-zinc-500 ${TAP}`}>
              <Paperclip className="w-5 h-5" />
            </button>
            <div className="flex-1 flex items-end bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 rounded-2xl px-3 py-2">
              <textarea
                ref={textareaRef}
                value={inputText}
                onChange={handleInputChange}
                placeholder={`Написать в ${currentApp?.name}…`}
                rows={1}
                className="flex-1 bg-transparent resize-none outline-none text-[14px] leading-snug placeholder:text-zinc-400 max-h-[140px]"
              />
              <button onClick={toggleVoiceInput} className={`p-1.5 ml-1 rounded-full ${TAP} ${isVoiceListening ? 'text-red-500 bg-red-500/10' : 'text-zinc-400'}`}>
